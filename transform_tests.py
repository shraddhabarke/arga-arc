...
class TestGrammarRepresentation(unittest.TestCase):
    ...
    def test_variable_initialization(self):
        color_variable = Variable(name="colorVar", node_type=Types.COLOR)
    ...
    def test_variable_initialization(self):
        var_node = Variable(name="colorVar", node_type=Types.COLOR)
        ...
        self.assertEqual(var_node.children, None)
        self.assertEqual(var_node.childTypes, [])
        self.assertEqual(var_node.values, [])
        applied_node = var_node.apply(None, None, None)
        self.assertIsInstance(applied_node, Variable)
        self.assertEqual(applied_node.childTypes, [])

    def test_update_color_mod(self):
        constant_color = Color.blue
        variable_color = Variable("X", Types.COLOR)
        ...
        node_const = constant_color.apply(
            self.task, constant_color.children, self.filter)
        node_var = variable_color.apply(
            self.task, variable_color.children, self.filter)
    ...
    def test_extend_node(self):
        ...
        new_instance = extend_node_instance.apply(
            self.task, extend_node_instance.children, self.filter)
        self.assertEqual(new_instance.nodeType, Types.TRANSFORMS)
        self.assertEqual(new_instance.code,
                         "extendNode(Dir.LEFT, Overlap.TRUE)")
    ...
    def test_update_color(self):
        ...
        new_instance = instance.apply(
            self.task, instance.children, self.filter)
        self.assertEqual(new_instance.nodeType, Types.TRANSFORMS)
    ...
    def test_get_all_values(self):
        ...
        # because there are 11 colors defined in the Color enum
        self.assertEqual(len(all_colors), 11)
    ...
    def test_transforms(self):
        ...
        self.assertEqual(move_node_instance.nodeType, Types.TRANSFORMS)
        self.assertEqual(move_node_instance.code,
                         "[moveNode(Dir.DOWN), moveNode(Dir.UP)]")
        # 1 for transforms + 2 for each updateColor
        self.assertEqual(move_node_instance.size, 4)
        self.assertEqual(len(move_node_instance.children), 2)

    def test_sequence_of_multiple_transforms(self):
        transforms_list = Transforms(UpdateColor(
            Color.green), Transforms(MoveNode(Dir.UP), NoOp()))
        self.assertEqual(transforms_list.nodeType, Types.TRANSFORMS)
        self.assertEqual(transforms_list.code,
                         "[updateColor(Color.green), [moveNode(Dir.UP), NoOp]]")
    ...
    def test_complex_transform_sequence(self):
        ...
        self.assertEqual(transforms_list.nodeType, Types.TRANSFORMS)
        self.assertEqual(
            transforms_list.code, "[updateColor(Color.blue), [moveNode(Dir.UP), [rotateNode(Rotation_Angle.CCW), [mirror(Mirror_Axis.Y_AXIS), [addBorder(Color.grey), NoOp]]]]]")
