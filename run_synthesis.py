...
def compare_graphs(transformed_nodes, true_output_graph):
    ...
    for iter in range(len(true_output_graph)):
        ...
        assert len(dict(transformed_nodes[iter]).keys()) == len(expected_nodes.keys())
...
def synthesize():
    ...
    while enumerator.hasNext():
        ...
        if program.values != []:
            ...
            errors, matches, mismatches, total_pixels = compare_graphs(program.values, output_graph)
            ...
            if errors == 0:
                ...
                print("expected:", expected_nodes)
                print("Filter Count:", filter_counter)
                return {program: matches}
            
            elif matches:   # Optimization: Ignore partial programs that cover same subset of nodes correctly
                ...
                for num in range(2, len(partial_solutions) + 1):
                    for subset in combinations(partial_solutions.keys(), num):
                        ...
                        #print("subset considered:", [program.code for program in subset])
                        #print([program.values for program in subset])
