...
class Task:
    ...
    def var_transform_values(self, filter: FilterASTNode, transformations: list):
        ...
        def generate_cartesian_product(input_graph, output_graph):
            ...
            out_dict = {
                node[0]: node[1] for node in output_graph.graph.nodes(data=True)
            }
            matching_nodes = [
                node[0]
                for node in in_dict
                if node[0] in out_dict
                and node[1]["color"] == out_dict[node[0]]["color"]
            ]
            ...
            cartesian_product = (
                combo
                for combo in product(colors, repeat=len(in_dict) - len(matching_nodes))
                if len(set(combo)) > 1
            )
        ...
        transformed_values = []
        for train_input, input_graph, output_graph in zip(
            self.train_input,
            self.input_abstracted_graphs_original[self.abstraction],
            self.output_abstracted_graphs_original[self.abstraction],
        ):
            ...
            for color in color_combo:
                for transformation in transformations:
                    input_graph.varcolor_apply_all(color, filter, transformation)
